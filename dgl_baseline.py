...
if __name__ == "__main__":
    ...
    print("Loading data")
    dataset = AsNodePredDataset(DglNodePropPredDataset("ogbn-products", root='/home/jason/DDP_GNN/dataset/'))
    g = dataset[0]
    ...
    for i in range(nprocs):
        p = mp.Process(target=run, args=(i, nprocs, devices, g, data, args.mode))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
